...
export default function StatusFilter({ selected, onChange }: StatusFilterProps) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className="flex gap-1.5 overflow-x-auto py-1 px-1 no-scrollbar flex-1">
        {MAIN_GROUPS.map((group) => (
          <button
            key={group}
            onClick={() => { onChange(group); setShowServiceMenu(false); }}
            className={`shrink-0 px-2 py-1 sm:px-3 sm:py-1.5 rounded-full text-[11px] sm:text-xs font-medium transition-all active:translate-y-px ${
              selected === group
      ...
      {/* Servicios dropdown — fuera del overflow container */}
      <div className="relative shrink-0" ref={menuRef}>
        ...
        {showServiceMenu && (
          <div className="absolute right-0 top-full mt-1 z-50 w-48 bg-card rounded-xl ring-1 ring-foreground/10 shadow-xl py-1">
